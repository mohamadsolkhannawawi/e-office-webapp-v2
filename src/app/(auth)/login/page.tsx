...
export default function LoginPage() {
  ...
  const redirectToDashboard = React.useCallback(
    (user: AuthUser) => {
      ...
      if (roles.includes("SUPER_ADMIN")) {
        ...
        console.warn("No matching role found, falling back to /dashboard");
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (logoutSuccess === "true") {
        toast.success("Berhasil keluar dari akun! Sampai jumpa kembali", {
          duration: 4000,
        });
  ...
  return (
    <div className="flex min-h-screen flex-col bg-bg-light font-sans text-gray-800 antialiased dark:bg-bg-dark dark:text-gray-200">
      ...
      <main className="flex grow items-center justify-center p-4 md:p-6">
        <div className="flex w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-card-light shadow-xl dark:bg-card-dark md:flex-row md:min-h-96">
          ...
          <div className="flex w-full flex-col items-center justify-center border-b border-gray-100 p-4 md:p-8 text-center dark:border-gray-700 dark:bg-gray-900 md:w-1/2 md:border-b-0 md:border-r">
            ...
            <h1 className="hidden md:block mb-3 text-2xl font-bold text-dark-navy dark:text-white">
              Aplikasi Persuratan Fakultas Sains dan Matematika
            </h1>
            <p className="hidden md:block max-w-sm text-sm leading-relaxed text-gray-500 dark:text-gray-400">
              Selamat datang di Portal Aplikasi UNDIP FSM. Silakan masuk untuk
              mengakses dasbor Anda.
            </p>
          </div>
      ...
      {/* Footer */}
      <footer className="flex w-full flex-col items-center justify-between px-6 py-6 text-center text-xs text-gray-500 dark:text-gray-400 md:flex-row md:text-left">
