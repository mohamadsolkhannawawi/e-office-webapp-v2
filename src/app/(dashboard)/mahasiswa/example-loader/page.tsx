...
export default function ExampleImplementation() {
  ...
  // Contoh 1: Pembuatan PDF
  const handleGeneratePDF = async () => {
    await generatePDF(async () => {
      // Simulasikan proses pembuatan PDF
      await delay(3000);

      // Mock pembuatan PDF
      const pdfBlob = new Blob(["Mock PDF Konten"], {
  ...
  // Contoh 2: Pengiriman Form dengan Loading Kustom
  const handleSubmitForm = async () => {
    await executeWithLoading(
      async () => {
        ...
        // Mock pemanggilan API
        const response = await fetch("/api/submit-form", {
          method: "POST",
          headers: { "Konten-Type": "application/json" },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Contoh Formulir */}
      <Card className="p-6">
        <h2 className="text-lg font-semibold mb-4">Contoh Formulir</h2>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Beasiswa</label>
            <Input
      ...
      {/* Aksi Tambahan */}
      <Card className="p-6">
        <h2 className="text-lg font-semibold mb-4">Operasi Halaman</h2>
        <Button onClick={handleLoadData} variant="outline" className="w-full">
          Muat Data (Loader Halaman)
        </Button>
      </Card>
      ...
      <Card className="p-6 bg-blue-50">
        ...
        <div className="space-y-2 text-sm">
          ...
          <p>
            ...
            <code>useAsyncOperation()</code> dengan custom loading key
          </p>
          <p>
            <strong>3. Page Loading:</strong> Menggunakan page loader untuk
            operasi yang memuat data halaman
          </p>
        </div>
      </Card>
    </div>
