...
function SuratDraftKonten() {
  ...
  // Sinkronkan jenisFilter saat URL berubah (navigasi sidebar antar halaman)
  useEffect(() => {
  ...
  const fetchApplications = useCallback(
    async (page: number, search: string, jenis: string, limit: number) => {
      // Tampilkan loading hanya untuk pengambilan data selain inisialisasi awal
      if (!isInitialLoad) {
      ...
      try {
        const { data, meta } = await getApplications({
          ...
          // Turunkan nilai dari parameter jenis (bukan state tertutup) agar selalu mutakhir saat navigasi
          excludeJenisBeasiswa:
            jenis !== "keperluan_lain" ? "keperluan_lain" : undefined,
        });
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Gagal memuat draft surat";
  ...
  const handleDeleteDraft = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDeleteError(
          errorData.error || "Gagal menghapus draft. Silakan coba lagi.",
        );
      }
    } catch (error) {
            ...
            <div className="px-4 py-12 text-center">
              <div className="flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-sm text-slate-400">
                  Belum ada draft yang tersimpan saat ini.
                </p>
              </div>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1 space-y-2">
                      ...
                      <p className="text-xs text-slate-500">
                        {app.updatedAt && !isNaN(new Date(app.updatedAt).getTime())
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    <Button
                      size="sm"
                      className="h-9 w-9 shrink-0 p-0 text-slate-400 hover:text-red-600 hover:bg-red-50"
                      variant="ghost"
                      title="Hapus Draft"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-slate-600 text-sm">
                        {app.updatedAt &&
                        !isNaN(new Date(app.updatedAt).getTime())
