...
async function getDashboardData(searchParams: SearchParams) {
    try {
        ...
        // 3. Fetch Action Required Count (Step 2 - Manajer TU)
        const actionRes = await fetch(
            `${apiUrl}/api/surat-rekomendasi/applications?currentStep=2&limit=1`,
            {
                headers: { Cookie: cookie || "" },
                cache: "no-store",
            },
        ...
        return {
            ...
            recentLetters: appsData.map((app: ApplicationSummary) => ({
                ...
                target: app.status === "COMPLETED" ? "Selesai" : "Manajer TU",
                status:
                    app.status === "PENDING"
                        ? "Menunggu Verifikasi"
