...
export default function DocumentManagementPage() {
  ...
  const handleCleanup = async (type: "all" | "orphaned" | "temp") => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Gagal melakukan cleanup",
      );
    } finally {
      setProcessing(false);
      setConfirmDialog({
    ...
    return (
      <div className="flex h-96 items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Memuat statistik dokumen...</p>
        </div>
      </div>
    );
  }
        ...
        <Card className="border-gray-200 shadow-sm rounded-3xl">
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-slate-500">Letter Instances</p>
                <p className="text-2xl font-bold text-slate-800">
            ...
            <div className="flex items-center gap-4">
              <div
                className={`rounded-full p-3 ${
                  duplicateCount > 0 ? "bg-amber-100" : "bg-green-100"
                }`}
              >
                <AlertTriangle
                  className={`h-6 w-6 ${
              ...
              <div>
                <p className="text-sm text-slate-500">Duplikat</p>
                <p className="text-2xl font-bold text-slate-800">
                  {duplicateCount}
                </p>
              </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col gap-3 p-4 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Hapus file lama, simpan hanya versi terbaru untuk setiap surat
                </p>
                {duplicateCount > 0 && (
            ...
            <Button
              ...
              className="w-full sm:w-auto bg-undip-blue hover:bg-undip-blue/90 rounded-xl"
            >
              {processing ? (
                <>
          ...
          <div className="flex flex-col gap-3 p-4 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Hapus file yang tidak memiliki record di database
                </p>
              </div>
            </div>
            <Button
          ...
          <div className="flex flex-col gap-3 p-4 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Hapus file temporary yang sudah lebih dari 1 jam
                </p>
              </div>
            </div>
            <Button
      ...
      {stats && Object.keys(stats.filesByLetterInstance).length > 0 && (
        <Card className="border-gray-200 shadow-sm rounded-3xl">
          ...
          <CardContent>
            ...
            {Object.keys(stats.filesByLetterInstance).length > 20 && (
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() =>
                confirmDialog.type && handleCleanup(confirmDialog.type)
              }
              disabled={processing}
