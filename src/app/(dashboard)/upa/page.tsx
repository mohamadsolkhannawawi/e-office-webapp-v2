...
async function getDashboardData(searchParams: SearchParams) {
    try {
        ...
        // 2. Fetch Pending Count (letters awaiting action at step 4)
        const pendingRes = await fetch(
            `${apiUrl}/api/surat-rekomendasi/applications?mode=pending&currentStep=4&limit=1`,
            {
                headers: { Cookie: cookie || "" },
                cache: "no-store",
            },
        ...
        // 3. Fetch Processed Count (letters processed by UPA - COMPLETED status for UPA)
        const processedRes = await fetch(
            `${apiUrl}/api/surat-rekomendasi/applications?status=COMPLETED&limit=1`,
            {
                headers: { Cookie: cookie || "" },
                cache: "no-store",
            },
