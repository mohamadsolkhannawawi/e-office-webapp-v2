...
export function SuperAdminDashboard({
  ...
  const pieData = (stats.usersByRole || []).map((role) => ({
    name: role.name.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()),
    value: role.count,
      ...
      <div>
        ...
        <p className="text-slate-500 text-lg leading-relaxed">
          {user?.name ? (
            <>
              Selamat datang{" "}
              <span className="text-undip-blue font-bold">{user.name}</span>,
              pantau dan kelola seluruh sistem E-Office FSM UNDIP dari sini.
            </>
          ) : (
        ...
        <CardHeader>
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="text-xs sm:text-sm text-slate-500">
              Menampilkan {recentActivities.length} aktivitas terakhir
            </div>
          </div>
        </CardHeader>
        <CardContentUI>
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {activity.role && (
                            ...
                            >
                              {formatRoleName(activity.role.name)}
                            </Badge>
                          )}
                        </div>
                        ...
                        <div className="space-y-1">
                          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm min-w-0">
                            ...
                            <span className="font-medium text-slate-700 wrap-break-word">
                          ...
                          {activity.letterInstance && (
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm min-w-0">
                              ...
                              <span className="text-slate-600 wrap-break-word">
                                {activity.letterInstance.letterType.name}
                              </span>
                              {activity.letterInstance.letterNumber && (
                                <>
                                  <span className="text-slate-400">•</span>
                          ...
                          {activity.note && (
                            <div className="flex items-start gap-2 text-sm">
                              ...
                              <span className="text-slate-600 italic wrap-break-word">
                                {activity.note}
                              </span>
                            </div>
                          )}
