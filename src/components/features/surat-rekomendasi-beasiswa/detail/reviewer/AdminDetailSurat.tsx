...
export function AdminDetailSurat({
  ...
  const isTerminalStatus =
    initialData?.status === "COMPLETED" || initialData?.status === "REJECTED";
  ...
  const hasResubmittedAfterRevision =
    initialData?.history?.some((h, idx, arr) => {
      ...
      const previousRevisionFromThisRole = arr.find((prevH, prevIdx) => {
        ...
        const prevRoleName =
          prevH.role?.name || prevActor?.role?.name || prevActor?.name;
        const currentRoleName =
          h.role?.name || currentActor?.role?.name || currentActor?.name;
  ...
  const canStaffEdit =
    (role === "supervisor-akademik" ||
      role === "manajer-tu" ||
      (role === "super-admin" && (currentStep === 1 || currentStep === 2))) &&
    canTakeAction;
  ...
  const handleAction = (type: "approve" | "revise" | "reject" | "publish") => {
  ...
  const handleConfirmAction = async (data: {
    ...
    try {
      ...
      const payload: {
        ...
        notes: data.reason,
        targetStep: data.targetRole ? roleToStep[data.targetRole] : undefined,
      };
      ...
      if (
        (role === "upa" || (role === "super-admin" && currentStep === 4)) &&
        upaLetterNumber
      ) {
          ...
          <RiwayatSurat
            riwayat={[
              ...(initialData?.history?.map((log) => {
                ...
                return {
                  senderRole: log.role?.name || log.actor?.name || "Sistem",
              ...
              {
                senderRole:
                  initialData?.createdBy?.mahasiswa?.user?.name || "Mahasiswa",
                ...
                time: initialData?.createdAt
                  ? new Date(initialData.createdAt).toLocaleTimeString(
                      "id-ID",
                      {
                        hour: "2-digit",
                        minute: "2-digit",
