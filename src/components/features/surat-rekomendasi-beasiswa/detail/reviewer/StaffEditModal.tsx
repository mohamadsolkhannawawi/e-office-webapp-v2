...
export function StaffEditModal({
  ...
  const [departemen, setDepartemen] = useState(initialValues?.departemen || "");
  ...
  const isCatatanFilled = catatan.trim().length > 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (namaLengkap.trim()) valuesPatch.namaLengkap = namaLengkap.trim();
      ...
      if (programStudi.trim()) valuesPatch.programStudi = programStudi.trim();
      if (tempatLahir.trim()) valuesPatch.tempatLahir = tempatLahir.trim();
      if (tanggalLahir.trim()) valuesPatch.tanggalLahir = tanggalLahir.trim();
      ...
      if (namaBeasiswa.trim()) valuesPatch.nama_beasiswa = namaBeasiswa.trim();

      await submitStaffEdit(applicationId, {
        namaBeasiswa: namaBeasiswa.trim() || undefined,
        values: Object.keys(valuesPatch).length > 0 ? valuesPatch : undefined,
      ...
      toast.error(
        err instanceof Error ? err.message : "Gagal memperbarui data surat.",
      );
      setStep("form");
    }
  };
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Perubahan berikut akan disimpan dan tercatat dalam riwayat
                    surat.
                  </p>
                </div>
              </div>
