  ...
  const addTambahanFiles = async (files: File[] | FileList) => {
    ...
    try {
      ...
      for (const file of arr) {
        ...
        try {
          const res = await uploadAttachment(letterId, file, "Tambahan");
          if (res) {
            setData((prev) => ({
              ...prev,
              lampiranTambahan: [
  ...
  return (
    <section aria-label="Upload Lampiran">
        ...
        <CardContent className="p-8 space-y-4">
          <div className="space-y-4">
            ...
            <p className="text-gray-500 -mt-2">
              {isKeperluanLain ? (
                <>
                  Wajib. Unggah dokumen pendukung yang relevan dengan keperluan
                  pengajuan (misalnya: surat pengantar, undangan, atau dokumen
                  resmi terkait). Format: PDF, JPG, PNG. Maks: 5MB/file.
                  <br />
                  <span className="text-xs text-red-500 font-semibold italic">
                    Catatan: Pastikan dokumen yang diunggah sesuai dengan
                    keperluan yang diajukan.
                  </span>
                  <br />
                  <span className="text-xs text-red-500">Maksimal 5 file.</span>
                </>
              ) : (
                <>
                  ...
                  <br />
                  <span className="text-xs text-red-500 font-semibold italic">
                    Catatan: Pastikan mengunggah KTM dan KHS terbaru.
                  </span>
                  <br />
                  <span className="text-xs text-red-500">Maksimal 5 file.</span>
                </>
              ...
              {uploadingMain ? (
                ...
                <>
                  ...
                  <div className="text-sm font-medium text-gray-700">
                    Seret &amp; lepas atau{" "}
                    <span className="text-[#007bff] font-bold">klik area</span>
                  </div>
                  <input
                    ref={mainInputRef}
                    type="file"
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-start">
                ...
                <Select value={selectedUtama} onValueChange={setSelectedUtama}>
                  <SelectTrigger className="h-9 w-36 text-sm sm:w-40">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              (() => {
                const filteredFiles = data.lampiranUtama.filter(
                  (f: LampiranFile) =>
                    selectedUtama === "Semua"
                      ? true
                      : getDisplayType(f) === selectedUtama,
                );
                ...
                return (
                  <div className="space-y-4">
                    {filteredFiles.map((f: LampiranFile, idx: number) => (
                      <div
                        key={`preview-u-${idx}`}
                        ...
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="text-xs text-gray-500 mt-1 flex items-center gap-2">
                              <span
                                className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                                  getDisplayType(f) === "File"
                                    ? "bg-red-100 text-red-700"
                                    : getDisplayType(f) === "Foto"
                              ...
                              <span>{formatSize(Number(f.size) || 0)}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleViewFile("utama", idx)}
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction
                                    onClick={() => handleDelete("utama", idx)}
      ...
      <Card className="mt-8 border-none shadow-sm bg-white rounded-3xl">
        <CardContent className="p-8 space-y-4">
          <div className="space-y-4">
            <p className="font-bold text-gray-800">Lampiran Tambahan</p>
            <p className="text-gray-500 -mt-2">
              Opsional. Tambahkan dokumen pendukung lainnya jika diperlukan.
              <br />
              <span className="text-xs text-red-500">Maksimal 3 file.</span>
            </p>
            ...
            <div
              ...
              onClick={() =>
                !uploadingTambahan && tambahanInputRef.current?.click()
              }
              onDragOver={(e) => {
                ...
                const filteredFiles = data.lampiranTambahan.filter(
                  (f: LampiranFile) =>
                    selectedTambahan === "Semua"
                      ? true
                      : getDisplayType(f) === selectedTambahan,
                );
                        ...
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleViewFile("tambahan", idx)}
                              className="text-xs flex items-center gap-1 rounded-3xl"
                            >
                              <Eye className="w-3 h-3" />
                              Lihat
