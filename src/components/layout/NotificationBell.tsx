...
export function NotificationBell() {
  ...
  const handleOpenChange = async (open: boolean) => {
    ...
    if (open) {
      setIsLoading(true);
      const data = await getNotifications({ limit: 500 });
  ...
  return (
    <Popover open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent
        className="w-[calc(100vw-16px)] sm:w-105 md:w-130 max-h-[85vh] p-0 shadow-2xl border-slate-200 rounded-xl overflow-hidden"
        align="end"
        sideOffset={8}
        avoidCollisions
      >
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${
                      !notification.isRead ? "bg-undip-blue" : "bg-transparent"
                    }`}
                  />
